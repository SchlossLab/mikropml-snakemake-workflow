...
def pattern_tame_wildcard(paramspace, wildcard):
    ...
    """
    return re.sub(
        f"{{((?!{wildcard})[a-zA-Z_0-9]*)}}", "{{\\1}}", paramspace.wildcard_pattern
    )
...
def get_paramspace_from_config(config):
    ...
    exclude_param_keys = (
        config["exclude_param_keys"]
        if "exclude_param_keys" in config
        and isinstance(config["exclude_param_keys"], Iterable)
        else list()
    )
    ...
    params_df = pd.DataFrame(
        list(it.product(*[v for v in conf_lists.values()])), columns=conf_lists.keys()
    )
